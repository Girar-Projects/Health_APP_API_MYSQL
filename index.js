...
app.get('/professional/personal-info/:id', authenticate, (req, res) => {
  ...
  if ( req.user.type !== 'professional') {
    ...
    connection.query('SELECT * FROM HealthProfessional WHERE id=?', [id], (err, results) => {
      if (err) return queryError(res, err);
      res.json(results);
    });
  }
...
app.put('/professional/personal-info/:id', authenticate, (req, res) => {
  ...
  if ( req.user.type !== 'professional') {
    ...
    connection.query('UPDATE HealthProfessional SET firstName=?, lastName=?, email=?, phoneNumber=?, address=?, city=?, state=?, zipCode=? WHERE LegalDocsID=?',
      [data.firstName, data.lastName, data.email, data.phoneNumber, data.address, data.city, data.state, data.zipCode, id],
      (err) => {
        if (err) return queryError(res, err);
...
app.post('/professional/experience-skill/:uid', authenticate, (req, res) => {
  ...
  if (  req.user.type !== 'professional') {
    ...
    connection.query('INSERT INTO ExperienceSkill (experienceTitle, experienceDescription, skillDescription, professionalId) VALUES (?, ?, ?, ?)',
      [data.experienceTitle, data.experienceDescription, data.skillDescription, uid],
      (err) => {
        if (err) return queryError(res, err);
...
app.post('/professional/bookmark', authenticate, (req, res) => {
  ...
  if (req.user.type !== 'professional') {
    ...
    connection.query('INSERT INTO Bookmarks (professionalId) VALUES (?)', [data.professionalId], (err) => {
      if (err) return queryError(res, err);
...
app.get('/professional/bookmark/:id', authenticate, (req, res) => {
  ...
  if (req.user.type !== 'professional') {
    ...
    connection.query('SELECT * FROM Bookmarks WHERE id=?', [id], (err, results) => {
      if (err) return queryError(res, err);
      res.json(results);
    });
  }
...
app.post('/professional/apply', authenticate, (req, res) => {
  ...
  if (req.user.type !== 'professional') {
    ...
    connection.query('INSERT INTO Applications (professionalId, jobId) VALUES (?, ?)', [data.professionalId, data.jobId], (err) => {
      if (err) return queryError(res, err);
...
app.get('/professional/my-applied', authenticate, (req, res) => {
  ...
  if (req.user.type !== 'professional') {
    ...
    connection.query('SELECT * FROM Applications WHERE professionalId=?', [id], (err, results) => {
      if (err) return queryError(res, err);
      res.json(results);
    });
  }
...
app.get('/professional/job/:id', authenticate, (req, res) => {
  ...
  if (req.user.type !== 'organization') {
    ...
    connection.query('SELECT * FROM JobPosts WHERE id=?', [id], (err, results) => {
      if (err) return queryError(res, err);
      res.json(results);
    });
  }
...
app.post('/organization/organization-info/:id', authenticate, (req, res) => {
  ...
  if ( req.user.type !== 'organization') {
    ...
    connection.query('UPDATE Organization SET name=?, email=?, description=?, phoneNumber=?, address=?, city=?, state=?, zipCode=? WHERE id=?',
      [data.name, data.email, data.description, data.phoneNumber, data.address, data.city, data.state, data.zipCode, id],
      (err) => {
        if (err) return queryError(res, err);
...
app.get('/organization/organization-info/:id', authenticate, (req, res) => {
  ...
  if ( req.user.type !== 'organization') {
    ...
    connection.query('SELECT * FROM Organization WHERE id=?', [id], (err, results) => {
      if (err) return queryError(res, err);
      res.json(results);
    });
  }
...
app.get('/organization/professional/:id', authenticate, (req, res) => {
  ...
  if (req.user.type !== 'organization') {
    ...
    connection.query('SELECT * FROM HealthProfessional WHERE LegalDocsID=?', [id], (err, results) => {
      if (err) return queryError(res, err);
      res.json(results);
    });
  }
...
app.get('/organization/my-jobs/:id', authenticate, (req, res) => {
  ...
  if ( req.user.type !== 'organization') {
    ...
    connection.query('SELECT * FROM JobPosts WHERE organizationId=?', [id], (err, results) => {
      if (err) return queryError(res, err);
      res.json(results);
    });
  }
...
app.post('/organization/my-jobs', authenticate, (req, res) => {
  ...
  if ( req.user.type !== 'organization') {
    ...
    connection.query('INSERT INTO JobPosts (organizationId, jobPosition, salary, deadline, jobType, numberOfEmployees, prerequisites, rolesAndResponsibilities) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [data.organizationId, data.jobPosition, data.salary, data.deadline, data.jobType, data.numberOfEmployees, data.prerequisites, data.rolesAndResponsibilities],
      (err) => {
        if (err) return queryError(res, err);
...
app.get('/organization/applied/:id', authenticate, (req, res) => {
  ...
  if ( req.user.type !== 'organization') {
    ...
    connection.query('SELECT * FROM Applications JOIN JobPosts ON Applications.jobId = JobPosts.id WHERE JobPosts.organizationId=?', [id], (err, results) => {
      if (err) return queryError(res, err);
      res.json(results);
    });
  }
...
app.post('/institution/legal-documents', authenticate, (req, res) => {
  ...
  if (req.user.type !== 'organization') {
    ...
    connection.query('INSERT INTO InstitutionLegalDocuments (institutionId, legalDocsId) VALUES (?, ?)',
      [data.institutionId, data.legalDocsId],
      (err) => {
        if (err) return queryError(res, err);
        res.json({ message: 'Legal document added to institution successfully' });
      });
  }
});
...
app.get('/institution/legal-documents/:id', authenticate, (req, res) => {
  ...
  if (req.user.type !== 'organization') {
    ...
    connection.query('SELECT * FROM InstitutionLegalDocuments JOIN LegalDocs ON InstitutionLegalDocuments.legalDocsId = LegalDocs.id WHERE InstitutionLegalDocuments.institutionId=?', [id], (err, results) => {
      if (err) return queryError(res, err);
      res.json(results);
    });
  }
